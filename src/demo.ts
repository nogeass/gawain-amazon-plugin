#!/usr/bin/env node
/**
 * Demo CLI for gawain-amazon-plugin
...
import { convertAmazonProduct, validateAmazonProduct } from './platform/amazon_adapter.js';
...
async function main(): Promise<void> {
  console.info('=== Gawain Amazon Plugin Demo ===\n');
  ...
  if (!validateAmazonProduct(rawProduct)) {
    console.error('Invalid product format. Expected Amazon product structure.');
    process.exit(1);
  }

  const productInput = convertAmazonProduct(rawProduct);
  console.info(`Product: ${productInput.title} (ASIN: ${productInput.id}, ${productInput.images.length} images)`);
